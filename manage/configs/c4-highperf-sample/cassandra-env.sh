...
# don't do mixed mode (young + old) scanning until the threshold
# 0 = scan continuously (CPU heavy)
...
# 2000ms has a little higher throughput with accompanying outliers
#JVM_OPTS="$JVM_OPTS -XX:MaxGCPauseMillis=2000"


# Cassandra does not benefit from biased locking
JVM_OPTS="$JVM_OPTS -XX:-UseBiasedLocking"

# allocate and zero (force fault) heap memory on startup
# somewhat redundant when JNA/mlockall is available but doesn't hurt
# and makes sure pages are faulted on systems without JNA
...
# JVM_EXTRA_OPTS="$JVM_OPTS -Dcassandra.metricsReporterConfigFile=metrics.yaml"

JVM_OPTS="$JVM_OPTS $JVM_EXTRA_OPTS"


# ALLOW NODES TO JOIN CLUSTER BEFORE 2 MINS
JVM_OPTS="$JVM_OPTS -Dcassandra.consistent.rangemovement=false"


# set scheduler to BATCH mode for long running process time slicing
